...
export class AnnouncementEngine {
    ...
    public async checkVersionUpdate(): Promise<boolean> {
        ...
        const currentVersion = vscode.extensions.getExtension('FreeRave.codetune')?.packageJSON?.version ?? '1.2.1';
    ...
    public async showWhatsNew(): Promise<void> {
        if (!this.context) { return; }
        const version = vscode.extensions.getExtension('FreeRave.codetune')?.packageJSON?.version ?? '1.2.1';
    ...
    public isNewVersion(): boolean {
        if (!this.context) { return false; }
        const current = vscode.extensions.getExtension('FreeRave.codetune')?.packageJSON?.version ?? '1.2.1';
